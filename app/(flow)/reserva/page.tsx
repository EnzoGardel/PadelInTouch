...
export default function ReservarPage() {
  ...
  const [loadingPay, setLoadingPay] = useState(false); // ← pago
  ...
  // Cargar canchas automáticamente al elegir sucursal (y autoseleccionar la primera)
  useEffect(() => {
  ...
  async function loadCourts(clubId: string) {
    try {
      ...
      // autoselecciona la primera cancha
  ...
  async function loadAvailableSlots(clubId: string, date: string) {
    ...
    try {
      ...
      // Soporta respuestas: array directo o { slots: [...] }
...
const handleProceedToPayment = async () => {
  ...
  if (missing.length > 0) {
    ...
    console.warn("[pago] faltan datos", { selectedLocation, selectedDate, selectedSlot, userDetails });
    return;
  }

  try {
    // 1) Crear reserva AUTOMÁTICA por sucursal/horario (sin especificar cancha)
    //    Asegurate que esta server action exista y asigne la cancha libre según stock.
    const auto = await createReservationAuto({
      branchId: selectedLocation!.id,   // id de la sucursal
      date: selectedDate!,
    ...
    console.log("[pago] createReservationAuto", auto);
    ...
    // 2) Crear preferencia de Mercado Pago y redirigir
    setLoadingPay(true);
    ...
    if (!prefRes.ok || !pref?.init_point) {
      console.error("Checkout MP error:", pref);
      alert(pref?.error || "No se pudo iniciar el pago. Revisá consola.");
      return;
    }
    ...
    alert("Error inesperado al iniciar el pago. Revisá la consola.");
  }
};
            ...
            {step === 2 && (
              <div className="space-y-4">
                <div className="mb-4">
                  ...
                  {loadingCourts ? (
                    <p className="text-sm text-slate-500">Cargando canchas…</p>
                  ) : selectedCourt ? (
                    <p className="text-sm text-slate-600">
                      Cancha seleccionada: <strong>{selectedCourt.name}</strong>
                    </p>
                  ) : (
                    <p className="text-sm text-slate-500">Seleccioná una sucursal para ver canchas.</p>
                  )}
                </div>

                <div>
                  <Label>Fecha de la reserva</Label>
                  <div className="flex mx-auto mt-2 p-2 rounded-lg border max-w-sm border-slate-200">
                    <DayPicker
                      mode="single"
                      locale={esLocale}
