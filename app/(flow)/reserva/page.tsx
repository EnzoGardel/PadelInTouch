...
export default function ReservarPage() {
  ...
  useEffect(() => {
    if (!branchIdQP) return;            // no hay query param
    if (!locations.length) return;      // esperamos a tener sedes
    if (selectedLocation) return;       // ya hay una seleccionada

    const found = locations.find(l => String(l.id) === String(branchIdQP));
    if (found) {
      setSelectedLocation(found);
      setStep(2);                       // directo al paso Fecha+Cancha
    }
